...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<section class="relative min-h-[80vh] flex items-end bg-black text-white overflow-hidden"><img${attr("src", project.hero.image)}${attr("alt", project.title)} class="absolute inset-0 w-full h-full object-cover opacity-90 scale-[1.05]"/> <div class="absolute inset-0 bg-black/60"></div> <div class="relative z-10 max-w-[1600px] mx-auto px-6 md:px-0 pb-20 w-full"><div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-end"><div class="lg:col-span-8 space-y-6"><img${attr("src", project.hero.logo)} class="h-10 md:h-14 object-contain" alt=""/> <h1 class="text-4xl md:text-6xl tracking-tight leading-[1.05]">${escape_html(store_get($$store_subs ??= {}, "$t", t)(project.overview.headlineKey))}</h1> <p class="text-white/70 text-lg max-w-2xl">${escape_html(store_get($$store_subs ??= {}, "$t", t)(project.overview.descriptionKey))}</p></div> <div class="lg:col-span-4 text-sm text-white/70 space-y-2"><p><strong>${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.meta.client"))}:</strong> ${escape_html(project.client)}</p> <p><strong>${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.meta.category"))}:</strong> ${escape_html(store_get($$store_subs ??= {}, "$t", t)(project.categoryKey))}</p> <p><strong>${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.meta.year"))}:</strong> ${escape_html(project.year)}</p> <p><strong>${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.meta.status"))}:</strong> ${escape_html(store_get($$store_subs ??= {}, "$t", t)(project.statusKey))}</p></div></div></div></section> <section class="max-w-[1600px] mx-auto px-6 md:px-0 py-24"><div class="grid grid-cols-1 lg:grid-cols-12 gap-16"><div class="lg:col-span-4"><h3 class="font-geist text-sm font-bold mb-6">${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.sections.scope"))}</h3> <ul class="space-y-3 text-black/70"><!--[-->`);
    const each_array = ensure_array_like(project.scopeKeys ?? []);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let key = each_array[$$index];
      $$renderer2.push(`<li>— ${escape_html(store_get($$store_subs ??= {}, "$t", t)(key))}</li>`);
    }
    $$renderer2.push(`<!--]--></ul></div> `);
    if (project.deliverableKeys?.length) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="lg:col-span-4"><h3 class="font-geist text-sm font-bold mb-6">${escape_html(store_get($$store_subs ??= {}, "$t", t)("portfolio.sections.deliverables"))}</h3> <ul class="space-y-3 text-black/70"><!--[-->`);
    ...
    if (project.gallery?.length) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<section class="max-w-[1600px] mx-auto px-6 md:px-0 py-24"><div class="grid grid-cols-1 md:grid-cols-2 gap-6"><!--[-->`);
    ...
    if (project.demos?.length) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<section class="max-w-[1600px] mx-auto px-6 md:px-0 py-32 space-y-32"><!--[-->`);
