...
function Hero($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    $$renderer2.push(`<section><div class="relative h-[100vh] rounded-[0px] overflow-hidden bg-black shadow-[0_0_60px_-20px_rgba(0,0,0,0.35)] flex items-end animate-[fadeIn_1.2s_ease-out]"><video src="videos/hero-video.mov" autoplay muted loop playsinline preload="auto" class="absolute inset-0 w-full h-full object-cover opacity-45"></video> <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/45 to-black/10"></div> <div class="grain-layer pointer-events-none absolute inset-0"></div> <div class="relative z-10 w-full"><div class="max-w-[1200px] mx-auto px-6 md:px-12 -translate-y-[100%] md:-translate-y-[75%]"><p class="font-inter text-sm text-white/60 mb-6">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.kicker"))}</p> <h1 class="font-calsans text-white text-[56px] sm:text-[72px] md:text-[96px] leading-[1.02] tracking-tight">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.title.main"))} <span class="text-white/40">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.title.muted"))}</span></h1> <p class="mt-8 font-inter text-lg md:text-xl text-white/70 max-w-[560px]">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.subtitle"))}</p> <div class="mt-12 flex items-center gap-6"><a href="/contact" class="inline-flex items-center gap-2 px-7 py-4 rounded-full bg-white text-black font-geist text-sm font-medium hover:bg-white/90 transition">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.cta.primary"))}</a> <a href="#selected-work" class="font-geist text-sm text-white/70 hover:text-white transition">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.cta.secondary"))}</a></div></div></div> <p class="absolute bottom-8 right-10 text-white/30 text-sm">${escape_html(store_get($$store_subs ??= {}, "$t", t)("hero.footer"))}</p></div></section>`);
...
function Blog($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<section class="mt-2 max-w-[1600px] mx-auto px-6 md:px-12 py-24"><div class="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16"><div class="lg:col-span-7"><h2 class="font-calsans text-5xl md:text-7xl tracking-tight leading-[1.05]">${escape_html(store_get($$store_subs ??= {}, "$t", t)("blog.section.titleA"))} <span class="text-black/40">${escape_html(store_get($$store_subs ??= {}, "$t", t)("blog.section.titleB"))}</span></h2></div> <div class="lg:col-span-5 flex items-end justify-between"><p class="font-inter text-lg text-black/60 max-w-sm">${escape_html(store_get($$store_subs ??= {}, "$t", t)("blog.section.subtitle"))}</p> <a href="/blog" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-black text-white text-sm font-geist hover:bg-black/90 transition">${escape_html(store_get($$store_subs ??= {}, "$t", t)("blog.section.cta"))} <span class="inline-block w-2 h-2 rounded-full bg-white"></span></a></div></div> <div class="grid grid-cols-1 lg:grid-cols-12 gap-6"><!--[-->`);
