		...
		{#each clients as client}
			...
			>
				<img
					src={client.logo}
					alt="Client logo"
					class="object-contain grayscale w-full px-6"
					style="max-height: {client.maxWidth ?? '200px'};"
				/>

				<!-- YEAR -->
				<span class="absolute bottom-6 right-6 text-xs font-inter text-black/40 font-bold">
					/{client.year}
				</span>
			</div>
		{/each}
