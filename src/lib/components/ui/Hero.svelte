...
<section class="">
	...
	>
		...
		<div class="relative z-10 w-full">
			<div class="max-w-[1200px] mx-auto px-6 md:px-12 -translate-y-[100%] md:-translate-y-[75%]">
				<p class="font-inter text-sm text-white/60 mb-6">{$t('hero.kicker')}</p>

				<h1
					class="font-calsans text-white text-[56px] sm:text-[72px] md:text-[96px] leading-[1.02] tracking-tight"
				>
					{$t('hero.title.main')}
					<span class="text-white/40">{$t('hero.title.muted')}</span>
				</h1>
		...
		<!-- FOOT NOTE -->
		<p class="absolute bottom-8 right-10 text-white/30 text-sm">{$t('hero.footer')}</p>
	</div>
</section>
